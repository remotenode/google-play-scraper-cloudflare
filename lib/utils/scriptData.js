...
function parseServiceRequests (response) {
  ...
  try {
    // Parse JavaScript object literal (not JSON) - this is safe as it's from Google's own response
    // The data structure contains unquoted keys which JSON.parse cannot handle
    const value = eval(`(${valueMatch[0]})`);
    return value;
  } catch (error) {
    debug('Error parsing service requests:', error);
